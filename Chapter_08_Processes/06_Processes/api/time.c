...
int clock_gettime(clockid_t clockid, timespec_t *time)
{
	...
	return syscall(CLOCK_GETTIME, clockid, time);
}
...
int nanosleep(timespec_t *request, timespec_t *remain)
{
	...
	return syscall(CLOCK_NANOSLEEP, CLOCK_REALTIME, 0, request, remain);
}
...
int timer_delete(timer_t *timer)
{
	...
	return syscall(TIMER_DELETE, timer);
}
...
int timer_gettime(timer_t *timer, itimerspec_t *value)
{
	...
	return syscall(TIMER_GETTIME, timer, value);
}
